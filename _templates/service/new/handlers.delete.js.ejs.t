...
import { delete<%= h.changeCase.pascal(h.inflection.singularize(name)) %> } from "../../../core/controllers/<%= h.changeCase.paramCase(name) %>/<%= h.inflection.singularize(name) %>.controller";
 const handler = async (event,context) => {
  ...
  try {
    const response = await delete<%= h.changeCase.pascal(h.inflection.singularize(name)) %>(event.pathParameters.id);
    if (response) {
       return callback(
        null,
        success(
